...
public class ServicesUtil<T> {
	...
	public T getObjectToUpdate(T objectToUpdate){
			for(Field field : objectToEval.getClass().getDeclaredFields()) {
				...
				try {
					...
					if(valueToChange == null) {
						...
						_log.info(" Updated  Field: "+name+" Value: "+valueUpdate+" Type: "+valueUpdate.getClass().getName());
					}else {

						_log.info(" Declared Field: "+name+" Value: "+valueToChange+" Type: "+valueToChange.getClass().getName());
					}

					valueUpdate = field.get(objectToEval);
					_log.info(" Last Mod Field: "+name+" Value: "+valueUpdate+" Type: "+valueUpdate.getClass().getName());
				} catch (IllegalArgumentException | IllegalAccessException err) {
					// TODO Auto-generated catch block
					_log.error(" Error at trying to build: "+objectToEval.getClass().getName()+" to update. Error: "+err.getMessage());
				}
			}
		return objectToEval;
	}
