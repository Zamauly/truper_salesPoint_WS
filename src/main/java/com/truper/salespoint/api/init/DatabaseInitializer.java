...
public class DatabaseInitializer {
	...
	CommandLineRunner initDatabase(ClienteRepository clienteRepository, ProductoRepository productoRepository,
		...
		Role userRole = new Role(Constants.ValidRole.USER_ROLE);
		Role adminRole = new Role(Constants.ValidRole.ADMIN_ROLE);
		...
		Producto initProducto2 = new Producto("TRP-02F3G", "Juego de Dados para matraca de 00pg hasta 12pg",
				"Dados Dif Tam", 12);
		ListaCompra initListaCompra1 = new ListaCompra(initClient1, "Lista Semanal",
				"Entregar directamente al cliente");
		ListaCompra initListaCompra2 = new ListaCompra(initClient1, "Lista Mensual",
				"Entregar directamente al cliente");
		ListaCompra initListaCompra3 = new ListaCompra(initClient2, "Lista Semanal",
				"Entregar a encargado en tienda \"Cosme Fulanito\"");

		return args -> {
			_log.info("Preloading " + roleRepository.save(userRole));
			_log.info("Preloading " + roleRepository.save(adminRole));
			...
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra1, initProducto1, 2)));
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra1, initProducto2, 1)));
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra2, initProducto1, 5)));
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra2, initProducto2, 2)));
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra3, initProducto1, 3)));
			_log.info(
					"Preloading " + listaDetalleRepository.save(new ListaDetalle(initListaCompra3, initProducto2, 1)));
		};
	}
}
